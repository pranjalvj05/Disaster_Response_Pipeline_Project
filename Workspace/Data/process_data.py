...
def clean_data(df):
    ...
    # Converting all values into binary (0 or 1)
    categories = (categories > 0).astype(int)

    # droping the original categories column 
    df.drop('categories', axis=1, inplace=True)

    # now concatenating the original dataframe with the new `categories` dataframe
    df = pd.concat([df, categories], axis=1)

    # now droping duplicates
    df.drop_duplicates(inplace=True)

    return df

def save_data(df, database_filename, table_name='Table_Clean'):
